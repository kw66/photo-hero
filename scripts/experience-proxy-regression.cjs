...
(async () => {
  ...
  assert.ok(worker.experienceDrawingUserPromptLines.includes("鉴定这张简笔画里的一个主体，输出客观识别 JSON。"));
  assert.ok(worker.experienceDrawingUserPrompt.includes("subjectCandidates"));
  assert.ok(worker.experienceDrawingUserPrompt.includes("visualEvidence"));
  assert.ok(worker.experienceDrawingUserPrompt.includes("不能把三叉戟叫飞镖"));
  ...
  assert.ok(String(built.messages[1].content[0].text).includes("客观识别 JSON"));
  assert.ok(String(built.messages[1].content[0].text).includes("subjectCandidates"));
  ...
  assert.ok(String(localBuilt.messages[1].content[0].text).includes("客观识别 JSON"));
  assert.ok(String(localBuilt.messages[1].content[0].text).includes("visualEvidence"));
